#ifndef PROGRAM_OPTION_TYPES_H
#define PROGRAM_OPTION_TYPES_H
...
public:
    ...
    Q_INVOKABLE void setValue(int value) {
        m_intervalIndex = (value - m_startingValue) / m_intervalLength;
        emit intervalIndexChanged();
    }
...
class ProgramIntegerMappingOption : public ProgramIntegerOption
{
    Q_OBJECT

    Q_PROPERTY(QList<int> mappedOptions MEMBER m_mapped_options NOTIFY mappedOptionsChanged)

signals:
    void mappedOptionsChanged();


public:
    using ProgramIntegerOption::ProgramIntegerOption;

    ProgramIntegerMappingOption& operator =(ProgramIntegerMappingOption& option)
    {
        ProgramIntegerOption::operator=(option);
        m_mapped_options = option.m_mapped_options;
        emit mappedOptionsChanged();
        return *this;
    }

    bool inline operator != (const ProgramIntegerMappingOption& option) const {
            return ProgramIntegerOption::operator !=(option) ||
                   (option.m_mapped_options != m_mapped_options);
    }

    QList<int> m_mapped_options {};

...
Item {
    ...
    property real screenOffset: 14
    property real screenLength: width - 2 * screenOffset
    property real percentageOfseparator: 1.0 / 15.0
    property real barHeight: 11
    property real swipeAreaHeight: 115
    ...
    property real maxOvershootFactor: 0.4
    property int maxRotationTime: 4
    property bool showOptionDescription: true

    // Internal variables
    property real anglePerIndex: visibleAngle / (2 * numberOfVisibleSideBars + 1)
    property real barAngle: anglePerIndex * (1 - percentageOfseparator)
    property real maxWheelAngle: maxOvershootFactor * barAngle
    property real minWheelAngle: -(options.length - 1) * anglePerIndex - maxOvershootFactor * barAngle
    property real halfVisibleAngleCosinus: Math.cos(visibleAngle / 2)
    // Virtual radius of the simulated wheel
    ...
    // Distance between observer and wheel
    ...
    Repeater {
        ...
        ShaderEffectSource {
            ...
            TextLabel {
                 ...
                 fontPixelSize: constants.textSizeF1 * (1 - 0.5 * relativePosition)
